...
Outputs are saved as a parquet file, where each row contains the following:
    ...
    - logits: (seq_len, top_k) tensor for each token (excluding the prompt), with the top K logit values
    - logits_vocab_idx: (seq_len, top_k) the corresponding vocabulary indices for each logit

Usage:
    python scripts/generate_teacher_logits.py experiments/distillation/logits_translation.cfg
    python scripts/generate_teacher_logits.py experiments/logits_translation.cfg -o language_code=ber max_samples=5000
"""
...
def compute_logits(
    ...
    with torch.no_grad():
        out = model(**inputs)
    topk, topk_indices = torch.topk(out.logits[0, :-1, :], k=top_k, dim=-1)
    return {
        "token_ids": inputs['input_ids'][0],
        "prompt_length": 1, # for the BOS token
        "logits": topk,
        "logits_vocab_idx": topk_indices,
    }
...
def generate_with_logits(
    ...
    topk, topk_indices = torch.topk(logits, k=top_k, dim=-1)
    return {
...
def generate_teacher_logits(config: DistillConfig) -> Dataset:
...
if __name__ == "__main__":
    ...
    dataset = generate_teacher_logits(config)

    teacher_short = _teacher_short_name(config.teacher_model)
    dataset_name = f"logits-{teacher_short}-{config.language_code}-{config.task}.parquet"
