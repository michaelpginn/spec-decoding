...
class NGramModel:
    ...
    def train(self, train: Dataset):
        ...
        for sentence in train["text"]:
            ...
            for idx in range(len(token_ids) - self.n):
                context = tuple(token_ids[idx : idx + self.n - 1])
                target = token_ids[idx + self.n]
                self.gram_freq[context][target] += 1
        self.ngram_vocab_size = sum(len(c) for c in self.gram_freq)
    ...
    def predict(self, tokens: list[int] | str):
        ...
        if isinstance(tokens, str):
            tokens = cast(list[int], self.tokenizer.tokenize(tokens))
    ...
    def __call__(
        ...
        if past_key_values is not None:
            ...
            full_seq = torch.concat([past_key_values[0], input_ids], dim=-1)
        else:
            full_seq = input_ids
        logits = self.predict(full_seq[0].tolist())
